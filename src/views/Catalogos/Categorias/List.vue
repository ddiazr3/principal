<template>
<div>
  <template v-if="permisosCategorias[0] ? (permisosCategorias[0].indexOf('index') == -1) : true">
        ...
        >
          <template #title>
            ...
            <btn
              ...
              link
              exact
              to="/catalogos/categorias/create"
              texto="Agregar Nueva Categoria"
              textoIcon="mdi-plus"
              v-if="permisosCategorias[0] ? (permisosCategorias[0].indexOf('create') != -1 ? true : false) : false"
            >
            </btn>
          </template>
          <v-container class="py-0">
            ...
            <v-simple-table fixed-header style="height: 525px;">
              ...
              <tbody>
                 <tr v-for="dato in categorias" :key="dato.id">
                    ...
                    <td>
                      <btn
                        ...
                        link
                        exact
                        :to="'/catalogos/categorias/edit/'+dato.idcrypt"
                        texto="Editar Categoria"
                        textoIcon="mdi-account-edit"
                        :idrecibir="dato.id"
                        v-if="permisosCategorias[0].indexOf('edit') != -1 ? true : false"
                      >
                      </btn>
                      <btn
                        v-if="permisosCategorias[0].indexOf('destroy') != -1 ? true : false"
                        color="error"
                        fab
                        small
...
export default {
  ...
  mounted () {
    ...
    this.getCategorias(url)
  },
  computed: {
    ...mapState('categoria', ['categorias', 'totalPage', 'page', 'permisosCategorias'])
  },
  methods: {
    ...mapActions('categoria', ['getCategorias','eliminarCategoria','exportarCategoria','importCategoria']),
